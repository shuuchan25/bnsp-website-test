    ...
    <section class="page-section">
        ...
        <div class="page-content">
            <div class="header d-flex align-items-center justify-content-between pb-lg-4 pb-2">
                <div class="">
                    <p class="">Hi Admin,</p>
                    <h3 class="">Release Schedule</h3>
                </div>
                <div class="">
                    <button type="button" class="primary-button" onclick="location.href='/admin/schedules/create'">Create Schedule</button>
                </div>
            </div>
            <div class="content-wrapper">
                ...
                @endif
                <form action="/admin/schedules" method="GET" id="search-form" class="w-100">
                    @csrf
                ...
                <div class="overflow-x-auto w-100">
                    ...
                    <table id="table-container" class="">
                        <tr class="bg-[#F6F6F6] text-sm ">
                            <th class="col-one">Title</th>
                            <th class="col-three">Category</th>
                            ...
                            <tr class="table-item">
                                <td class="">
                                    <div class="first-column">
                                        <p class="first-p">{{ $schedule->name }}</p>
                                    </div>
                                </td>
                                <td class="">{{ optional($schedule->category)->name }}</td>
                                <td class=""><img src="{{ Storage::url($schedule->image) }}" alt="" class="image-card"></td>
                                <td class="">
                                    <div class="action-buttons">

                                        <button class="" onclick="location.href='/admin/schedules/{{ $schedule->slug }}'">
                                        ...
                                        <button class="" onclick="location.href='/admin/schedules/{{ $schedule->slug }}/edit'">
                                        ...
                                        <form action="/admin/schedules/{{ $schedule->slug }}" method="POST"
...
<script>
    $(document).ready(function () {
        $('#search-input').on('input', function () {
            ...
            if (query.length >= 2) {
                $.ajax({
                    url: '/admin/schedules', // Gunakan rute yang sama dengan halaman index
                    method: 'GET',
                    data: { search: query },
                    success: function (data) {
                        $('#table-container').html(data); // Menampilkan hasil penSearchan di div dengan id "table-container"
                    }
                });
            } else {
                // Tampilkan konten asli jika kotak penSearchan kosong
                $.ajax({
                    url: '/admin/schedules',
                    method: 'GET',
                    success: function (data) {
